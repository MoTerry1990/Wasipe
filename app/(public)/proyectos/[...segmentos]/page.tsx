...
/**
 * Búsqueda con la dirección bonita: /proyectos/departamento/miraflores.
 *
...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  return metadatosDeBusqueda('project', combinados);
}

export default async function ProyectosPorSegmentos({ params, searchParams }: Props) {
  const { segmentos } = await params;
  const combinados = comoParametros(segmentos, await searchParams);
  ...
  return <PaginaDeBusqueda operacion="project" params={combinados} />;
}
