...
export function Modal({
  ...
  return (
    <dialog
      ...
      className={cn(
        'w-[min(32rem,calc(100vw-2rem))] rounded-marca border border-linea bg-white p-0',
    ...
    >
      <div className="flex items-start justify-between gap-4 border-b border-linea p-5">
        ...
        <button
          type="button"
          onClick={alCerrar}
          className="-m-1 rounded-lg p-1 text-tinta-40 transition-colors hover:bg-niebla hover:text-tinta"
        >
      ...
      {pie && <div className="flex justify-end gap-2.5 border-t border-linea p-5">{pie}</div>}
    </dialog>
  );
}
