 ...
 *
 * Pasó de verdad. En el sprint 22, el paso 1 de `/publicar` mostraba en
 * pantalla:
 *
 ...
 * `z.string()` de antes, y ahí habla Zod. El mismo patrón se repite en
 * varios campos más.
 *
 * Arreglarlos uno por uno dejaría el próximo sin cubrir. Esto cubre todos,
 * incluidos los que todavía no existen.
 *
 * Se importa una sola vez, desde `lib/validacion/aviso.ts`, para que quede
 * puesto antes de que se construya cualquier esquema.
 */
z.config({
  customError: (asunto) => {
    switch (asunto.code) {
